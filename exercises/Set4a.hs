...
longest :: Ord a => [[a]] -> [a]
longest xs = head(sortByFirst (getShortestLists xs))

getShortestLists :: [[a]] -> [[a]]
getShortestLists xs = filter (\x -> length x==getShortestLength xs) xs
...
getShortestLength :: [[a]] -> Int
getShortestLength = length . last . sortByLength
...
incrementKey _ [] = []
incrementKey key (value:xs) 
    | fst value == key = (fst value, snd value + 1) : incrementKey key xs
    | otherwise = value : incrementKey key xs
