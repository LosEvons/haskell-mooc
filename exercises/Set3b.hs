...
buildList start count end = buildListHelp start count [end]

buildListHelp :: Int -> Int -> [Int] -> [Int]
buildListHelp start count xs
    | count == 0 = xs
    | otherwise = buildListHelp start (count-1) (start:xs)
...
sumsHelp i list 
  | i == 0 = list
  | otherwise = sumsHelp (i-1) (sumForm i:list)

sumForm :: Int -> Int
sumForm n = div (n*(n+1)) 2
...
mylast :: a -> [a] -> a
mylast def [] = def
mylast def xs = mylastHelp xs


mylastHelp :: [a] -> a
mylastHelp [x] = x
mylastHelp (x:xs) = mylastHelp xs
